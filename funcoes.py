...
Módulo....: Página das Funções do sistema
...
def criar_conta(cpf):
    ...
    ## 3 - Número da Conta
    ## 4 - Nome da Conta
    conta_corrente = [0, """""", nAgencia, nConta, f"Conta corrente {nConta}"]
...
def saque(valor, saldo, extrato, num_saques, total_saques):
    ...
    if num_saques < nLmte_Saq and valor <= nLmte_drio and valor <= saldo and total_saques + valor < nLmte_drio + 1:
        saldo -= valor
        extrato += f"R${valor:.2f} D \n"
        num_saques += 1
        total_saques += valor
